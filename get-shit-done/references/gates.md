...
| Phase | Gate Type | Required Artifacts | Validation Criteria | Failure Behavior |
|-------|-----------|-------------------|---------------------|------------------|
| Research → Planning | Pre-Flight | RESEARCH.md | No unresolved `[NEEDS DECISION]` items; all claims tagged with provenance | Block planning; surface unresolved questions |
| Planning → Checking | Revision | PLAN.md | All tasks have files, action, verify, done; MECE coverage of requirements | Route back to planner with structured issue list |
| Checking → Execution | Escalation | Checker output | Issue count = 0 OR user explicitly approves proceeding | Surface choices: proceed / adjust / abandon |
| Execution → Verification | Pre-Flight | SUMMARY.md, committed code | All tasks have commits; SUMMARY.md exists with deviation log | Block verification; report missing artifacts |
| Verification → Complete | Revision | VERIFICATION.md | All must-haves pass L1-L3 checks; no BLOCKER gaps | Route to gap-closure planning if gaps found |
| Gap Closure → Re-verify | Pre-Flight | Gap-closure PLAN.md | Gap plan addresses root cause, not just symptom | Block execution; require root-cause analysis |
| Any phase | Abort | N/A | Config parseable, state directory exists, plan index readable | Stop immediately with diagnostic message |
