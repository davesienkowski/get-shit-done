...
- **Global scope**: `D:/Repos/get-shit-done/.cursor/notes/{YYYY-MM-DD}-{slug}.md` — fallback when no `.planning/`, or when `--global` flag is present
...
2. Ensure the notes directory exists (`.planning/notes/` or `D:/Repos/get-shit-done/.cursor/notes/`)
...
2. Glob `D:/Repos/get-shit-done/.cursor/notes/*.md` (if directory exists) — global notes
...
Global (D:/Repos/get-shit-done/.cursor/notes/):
...
2. **No `.planning/`**: Falls back to global `D:/Repos/get-shit-done/.cursor/notes/` — works in any directory
